...
<div class="max-w-7xl mx-auto space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
    ...
    <div class="flex items-center justify-between">
        <div class="flex items-center gap-4">
            ...
            <div>
                <h1 class="text-xl font-extrabold text-slate-950 tracking-tight">Müşteriyi Düzenle</h1>
                <p class="text-slate-500 text-sm mt-1">Müşteri bilgilerini güncelleyin.</p>
            </div>
        </div>
    </div>
            ...
            <div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <label class="relative flex-1 cursor-pointer group">
                        <input type="radio" name="type" value="legal" {{ $customer->type === 'legal' ? 'checked' : '' }} class="hidden peer">
                    ...
                    <label class="relative flex-1 cursor-pointer group">
                        <input type="radio" name="type" value="individual" {{ $customer->type === 'individual' ? 'checked' : '' }} class="hidden peer">
                        <div class="p-6 rounded-xl border-2 border-slate-50 bg-slate-50/50 peer-checked:border-indigo-600 peer-checked:bg-indigo-50 transition-all duration-300">
                            <div class="flex items-center gap-4">
        ...
        <div class="bg-white rounded-md p-8 border border-slate-100 shadow-sm">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div class="col-span-2">
                    ...
                    <input type="text" name="company_name" required value="{{ old('company_name', $customer->company_name) }}"
                ...
                <div class="col-span-2">
                    ...
                    <input type="text" name="legal_title" value="{{ old('legal_title', $customer->legal_title) }}"
                        class="w-full h-12 px-5 rounded-2xl bg-slate-50 border border-slate-100 text-sm font-bold focus:outline-none focus:ring-4 focus:ring-indigo-600/5 focus:border-indigo-600 transition-all"
                ...
                <div>
                    ...
                    <input type="email" name="company_email" value="{{ old('company_email', $customer->company_email) }}"
                        class="w-full h-12 px-5 rounded-2xl bg-slate-50 border border-slate-100 text-sm font-bold focus:outline-none focus:ring-4 focus:ring-indigo-600/5 focus:border-indigo-600 transition-all"
                ...
                <div>
                    ...
                    <input type="text" name="category" value="{{ old('category', $customer->category) }}"
                        class="w-full h-12 px-5 rounded-2xl bg-slate-50 border border-slate-100 text-sm font-bold focus:outline-none focus:ring-4 focus:ring-indigo-600/5 focus:border-indigo-600 transition-all"
            ...
            <div class="bg-white rounded-md p-8 border border-slate-100 shadow-sm h-full">
                ...
                <div class="space-y-6">
                    <div>
                        ...
                        <input type="text" name="mobile_phone" value="{{ old('mobile_phone', $customer->mobile_phone) }}"
                            class="w-full h-12 px-5 rounded-2xl bg-slate-50 border border-slate-100 text-sm font-bold focus:outline-none focus:ring-4 focus:ring-indigo-600/5 focus:border-indigo-600 transition-all"
                            placeholder="05xx xxx xx xx">
                    </div>
                    <div>
                        <label class="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2 px-1">Sabit Tel</label>
                        <input type="text" name="landline_phone" value="{{ old('landline_phone', $customer->landline_phone) }}"
                            class="w-full h-12 px-5 rounded-2xl bg-slate-50 border border-slate-100 text-sm font-bold focus:outline-none focus:ring-4 focus:ring-indigo-600/5 focus:border-indigo-600 transition-all"
        ...
        <div class="bg-white rounded-md p-8 border border-slate-100 shadow-sm">
            ...
            <div>
                ...
                <textarea name="address" rows="3" 
                    ...
                    placeholder="Mahalle, Cadde, Sokak, Kapı No vb.">{{ old('address', $customer->address) }}</textarea>
            </div>
        </div>
