...
<div class="max-w-7xl mx-auto space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
    ...
    <div class="flex items-center justify-between">
        <div class="flex items-center gap-4">
            ...
            <div>
                <h1 class="text-xl font-extrabold text-slate-950 tracking-tight">Ürünü Düzenle</h1>
                <p class="text-slate-500 text-sm mt-1">Ürün detaylarını güncelleyin.</p>
            </div>
        </div>
    </div>
    ...
    <form action="{{ route('products.update', $product) }}" method="POST" enctype="multipart/form-data" class="space-y-6"
        x-data="{
            ...
            buyingPriceExcl: {{ $product->buying_price ?? 0 }},
            buyingPriceIncl: 0,
            sellingPriceExcl: {{ $product->price }},
            sellingPriceIncl: 0,
            stockTracking: {{ $product->stock_tracking ? 'true' : 'false' }},
            criticalStockAlert: {{ $product->critical_stock_alert ? 'true' : 'false' }},
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div class="col-span-2">
                            ...
                            <textarea name="description" rows="3" 
                                ...
                                placeholder="Ürün hakkında detaylı açıklama...">{{ old('description', $product->description) }}</textarea>
                        </div>
                    </div>
                </div>
                                    ...
                                    <div class="relative">
                                        <input type="number" step="0.01" name="buying_price" x-model="buyingPriceExcl" @input="calculatePrices('buying', 'excl')"
                                         ...
                                         <select name="buying_currency" class="absolute right-0 top-0 h-full w-14 bg-transparent border-none text-xs font-bold text-slate-500 focus:ring-0 text-center">
                                            <option value="TRY" {{ $product->buying_currency === 'TRY' ? 'selected' : '' }}>TRY</option>
                                            <option value="USD" {{ $product->buying_currency === 'USD' ? 'selected' : '' }}>USD</option>
                                            <option value="EUR" {{ $product->buying_currency === 'EUR' ? 'selected' : '' }}>EUR</option>
                                        </select>
                                    </div>
                                </div>
                                <div>
                                ...
                                <div>
                                    ...
                                    <div class="relative">
                                        ...
                                        <select name="selling_currency" class="absolute right-0 top-0 h-full w-14 bg-transparent border-none text-xs font-bold text-slate-500 focus:ring-0 text-center">
                                            <option value="TRY" {{ $product->selling_currency === 'TRY' ? 'selected' : '' }}>TRY</option>
                                            <option value="USD" {{ $product->selling_currency === 'USD' ? 'selected' : '' }}>USD</option>
                                            <option value="EUR" {{ $product->selling_currency === 'EUR' ? 'selected' : '' }}>EUR</option>
                                        </select>
                                    </div>
                                </div>
                                <div>
                    ...
                    <div x-show="stockTracking" x-collapse>
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div>
                                ...
                                <input type="number" name="critical_stock_quantity" x-bind:disabled="!criticalStockAlert" value="{{ old('critical_stock_quantity', $product->critical_stock_quantity) }}"
            ...
            <div class="space-y-6">
                    ...
                    <div class="flex flex-col items-center justify-center w-full" x-data="{ 
                        ...
                        <input type="hidden" name="delete_image" x-model="deleteImage">
                         <label for="dropzone-file" class="flex flex-col items-center justify-center w-full h-64 border-2 border-slate-200 border-dashed rounded-2xl cursor-pointer bg-slate-50 hover:bg-slate-100 transition-colors relative overflow-hidden group">
                            ...
                            <input id="dropzone-file" type="file" name="image" class="hidden" accept="image/*" @change="const file = $event.target.files[0]; if(file){ const reader = new FileReader(); reader.onload = (e) => { imagePreview = e.target.result; deleteImage = false; }; reader.readAsDataURL(file); }" />
                        </label>
                        <button type="button" x-show="imagePreview" @click="imagePreview = null; deleteImage = true; document.getElementById('dropzone-file').value = ''" class="mt-3 text-xs font-bold text-rose-500 hover:text-rose-700 flex items-center gap-1 transition-colors">
                ...
                <div class="bg-white rounded-md p-4 md:p-8 border border-slate-100 shadow-sm">
                    ...
                    <select name="status" class="w-full h-12 px-5 rounded-2xl bg-slate-50 border border-slate-100 text-sm font-bold focus:outline-none focus:ring-4 focus:ring-indigo-600/5 focus:border-indigo-600 transition-all appearance-none cursor-pointer">
                        <option value="active" {{ old('status', $product->status) === 'active' ? 'selected' : '' }}>Aktif (Yayında)</option>
                        <option value="passive" {{ old('status', $product->status) === 'passive' ? 'selected' : '' }}>Pasif (Gizli)</option>
                    </select>
                </div>
            </div>
        </div>
